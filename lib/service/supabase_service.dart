...
class SupabaseHelper {
  ...
  // addInventory(ItemModel data) async {
  ...
  // // request
  addRequest(RequestModel data) async {
  ...
  Future<List<RequestModel>> getRequests() async {
    ...
    final query =
        Query(where: [Where('id').isExactly(supabase.auth.currentUser!.id)]);
    final requestItems = await Repository().get<RequestModel>(query: query);

    return requestItems;
  }

  updateRequest(RequestModel item) async {
    await Repository().upsert<RequestModel>(item);
    // await supabase
    //     .from('request')
    //     .select()
    //     .eq("id", item.id)
